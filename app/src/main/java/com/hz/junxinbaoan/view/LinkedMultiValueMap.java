...
public class LinkedMultiValueMap<K, V> implements MultiValueMap<K, V> {
    ...
    protected Map<K, ArrayList<V>> mSource = new LinkedHashMap<K, ArrayList<V>>();
    ...
    @Override
    public void set(K key, ArrayList<V> values) {
        // 移除Key，添加List<V>
        mSource.remove(key);
        add(key, values);
    }
    ...
    @Override
    public void set(Map<K, ArrayList<V>> map) {
    ...
    public V getValue(K key, int index) {
        ArrayList<V> values = mSource.get(key);
